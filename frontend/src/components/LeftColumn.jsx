        ...
        <div key={index} className="flex items-start space-x-4 p-4 border-b border-gray-300">
          <div className="flex-1">
            <button
              ...
              className="text-xl font-semibold  font-headline text-gray-900 hover:text-hover w-full text-left"
            >
      ...
      {showLoginDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-80">
            <h2 className="text-xl font-semibold font-headline text-gray-800">Login Required</h2>
            <p className="text-gray-700 mt-4">You need to be logged in to read the article.</p>
            <div className="mt-6 flex justify-between">
              <button
                onClick={() => setShowLoginDialog(false)}
                className="px-4 py-2 font-headline bg-gray-500 text-white rounded-md hover:bg-gray-600"
              >
                Close
              </button>
              <Link
                to="/login"
                className="px-4 py-2 font-headline bg-sky-600 text-white rounded-md hover:bg-sky-700"
              >
                Login
              </Link>
            </div>
